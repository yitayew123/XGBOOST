...
categorical_cols = ['EmploymentStatus']  # Columns that need categorical encoding

# Set Streamlit app configuration before rendering anything
st.set_page_config(page_title="🚗 EV Car Purchase Prediction 🚗", layout="wide")

# ================== LOAD MODEL ARTIFACTS ==================
@st.cache_resource(show_spinner=True)  # Cache loaded artifacts to avoid reloading every time
def load_artifacts(path="model_artifacts.pkl"):
    """Load model, scaler, and encoders from a pickle file."""
    artifacts = joblib.load(path)  # Load pickle file containing model & preprocessors
    return artifacts

# Load model artifacts
artifacts = load_artifacts()
model = artifacts.get("model", None)        # Trained machine learning model
scaler = artifacts.get("scaler", None)      # Scaler for feature normalization
encoders = artifacts.get("encoders", {})    # Dictionary of label encoders for categorical features

# Stop execution if required artifacts are missing
...
if 'reset_counter' not in st.session_state:
    st.session_state.reset_counter = 0  # Track how many times inputs are reset

def reset_inputs():
    """Reset all form input values to default settings."""
...
def user_input_features():
    """Render the input fields for user to enter feature values."""
    col1, col2, col3 = st.columns(3)  # Split form into 3 columns
    ...
    with col3:
        ...
        le = encoders.get('EmploymentStatus')  # Get EmploymentStatus encoder
        if le is not None:
            st.selectbox("Employment Status", le.classes_, key="employment_status")
        else:
            st.error("EmploymentStatus encoder not found.")

# Call function to render form
user_input_features()

# ================== BUTTONS ==================
col_pred, col_clear = st.columns([1, 1])
with col_pred:
    predict_button = st.button("Predict")  # Button to run prediction
with col_clear:
    clear_button = st.button("Clear", on_click=reset_inputs)  # Button to reset form
...
if predict_button:
    ...
    prediction_proba = model.predict_proba(input_scaled)[0][prediction]  # Confidence score
    ...
    if prediction == 1:
        ...
        # Negative prediction (No purchase)
        st.markdown(
            f"""
